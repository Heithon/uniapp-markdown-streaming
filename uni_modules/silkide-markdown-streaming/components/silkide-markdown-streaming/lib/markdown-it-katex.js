"use strict";
// ESM build for browser runtime: avoid CommonJS exports/require.
// KaTeX instance will be taken from options.katex or window.katex.
/**
...
function inlineMath(state, silent) {
  ...
  const tokensArr = state.tokens;
  const lastToken =
    tokensArr && tokensArr.length ? tokensArr[tokensArr.length - 1] : undefined;
...
function default_1(md, options) {
  const katex =
    ...
    (typeof window !== "undefined" ? window.katex : undefined);
  ...
  const katexInline = (latex) => {
    const displayMode = /\\begin\{(align|equation|gather|cd|alignat)\}/gi.test(
      latex
    );
    try {
  ...
  if (enableFencedBlocks) {
    ...
    md.renderer.rules.fence = function (tokens, idx, options, env, self) {
      ...
      if (
        ...
        return (
          originalFenceRenderer?.call(this, tokens, idx, options, env, self) ||
          ""
        );
      }
    };
